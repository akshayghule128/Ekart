pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/akshayghule128/Ekart.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t akshayghule1/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u akshayghule1 -p ${dockerhubpwd}'}
                   sh 'docker push akshayghule1/ekart:latest'
                }
            }
        }
